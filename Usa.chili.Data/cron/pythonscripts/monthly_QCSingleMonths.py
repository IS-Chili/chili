...
# It is compatible (tested) with Python 2.7.17 with numpy 1.16.5 and matplotlib 2.2.3 as well as Python 3.7.6 with numpy 1.18.1 and matplotlib 3.1.3
...
# Execution time will vary depending on the amount of data and server load but has generally taken 3-3.5 hours on the development sever.
# RAM usage for this script is around 500MB.
# Storage space will vary but may approach 1.5GB for 6 months of plots.
...
def main(inputStations):
    ...
    # Calendar number, not an index number.
    ...
    # Current leap years. Could be modified to calculate them rather than having to update this list 
    ...
    for mo in PlotMonths:
        ...
        if monthNum > endMonth: # This will only work for a years worth of plots
        ...
        for s in range(len(StationNames)):
            ...
            print('Query time for ' + mo + '. Station = ' + StationNames[s] + ': '  + str(elapsed_time))
            ...
            for row in result:
                ...
                #year = row[0] # Can be used for output directory structure
                #month = row[1] # Can be used for output directory structure
                day = row[2]
                hour = row[3]
                minute = row[4]

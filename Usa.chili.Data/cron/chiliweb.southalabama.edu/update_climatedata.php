#!/usr/bin/php
<?php
// 2012-02-15 dnb Original version
// 2012-02-22 dnb Added support for 1.5m Air temperature mean
...
$qry_sum = 'select round(avg(AirT_2m),2) as MeanTemp2m, round(avg(AirT_1pt5m),2) as MeanTemp1pt5m, round(sum(Precip_TB3_Tot),2) as Precip_TB3, round(sum(Precip_TX_Tot),2) as Precip_TX from %s_202 where TS >= "%s" and TS < "%s"';
$qry_ins = 'insert ignore climatedata values("%s","%s","%s","%01.2f","%01.2f","%01.2f","%01.2f","%01.2f")';

//
// Determine whether to update previous month or the specified year and month
//
if ($argc == 1) {
...
foreach ($station_arr as $station) {
  ...
  $collected = $row[0];
  if ($collected > 0) {
    //echo "There are $collected minute data records for $station during $year-$month\n";
    ...
    $query = sprintf($qry_ins,$year,$month,$station,$row['MeanTemp2m'],$row['MeanTemp1pt5m'],$row['Precip_TB3'],$row['Precip_TX'],$pctcoll);
    $result = mysql_query($query);
    if (!$result) {
      die("Unable to insert $year,$month,$station row into climatedata table, \$query : $query\n");
    }
  }
}
